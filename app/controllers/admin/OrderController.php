...
class OrderController extends AppController
{
    ...
    public function indexAction()
    {
        ...
        $orders = \R::getAll("SELECT `order`.`id`, `order`.`user_id`, `order`.`status`, `order`.`date`, `order`.`update_at`, `order`.`currency`, `user`.`name`, 
    ...
    public function viewAction()
    {
        ...
        $order = \R::getRow("SELECT `order`.*, `user`.`name`, `user`.`email`, `user`.`address`,
                             ROUND(SUM(`order_product`.`price`), 2) AS `sum` FROM `order`
                             JOIN `user` ON `order`.`user_id` = `user`.`id`
                             JOIN `order_product` ON `order`.`id` = `order_product`.`order_id`
                             WHERE `order`.`id` = ?
